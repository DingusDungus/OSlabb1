...
pthread_mutex_t lock =
    PTHREAD_MUTEX_INITIALIZER;  // Lock to hinder two professors reading or
                                // grabbing chopsticks on the same time
...
int chopstickScheduler(int id)  // Scheduling function which decides the order
                                // professors eat in so they don't deadlock
{
    if (id == professorTurn && cycleActive == 1) {
        if (id == 0 && (chopsticks[id] == 1 && chopsticks[4] == 1)) {
            ...
            if (professorTurn == 5) {
                professorTurn = 0;
            }  // Circular so it restarts at index 0 (i.e., 1->3->0)
            else if (professorTurn == 6) {
                professorTurn = 1;
            }  // Circular so it restarts at index 1 (i.e., 2->4->1)
            professorTurn += 2;
        }
        return 1;
    } else {
...
void* child(void* params) {
    ...
    int randomSleep =
        (rand() % 8) +
        2;  // Gives initial sleep random time between 2 and 10 seconds
    sleep(randomSleep);
    ...
    while (leftGrabbed == 0) {
        ...
        if (grabLeft(args->id) == 1) {
            ...
            randomSleep = (rand() % 2) + 1;  // Random time between 1 - 3
                                             // seconds
            sleep(randomSleep);
        } else {
            fflush(stdout);
    ...
    while (rightGrabbed == 0) {
        ...
        if (grabRight(args->id) == 1) {
            ...
            randomSleep =
                (rand() % 10) + 10;  // Random wait from 10 - 20 seconds
            sleep(randomSleep);
        } else {
            fflush(stdout);
        }
...
int main(int argc, char** argv) {
    ...
    initChopsticks();  // Inits chopsticks before other threads start working on
                       // it
